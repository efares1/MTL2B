...
let rec rem_rsts = function
  ...
  | And(And(c1,c2),c3) -> rem_rsts (And (c1, And(c2,c3)))
  | And(c1,c2) ->
...
let rec rem_unch = function
  ...
  | And(And(c1,c2),c3) -> rem_unch (And (c1, And(c2,c3)))
  | And(c1,c2) ->
...
let get_unch a = List.filter (fun x -> String.get x 1 = 'u') (get_clk a)

(* reset des horloges xri marquées Reset et
   des horloges xui non marquées Unch *)
let add_rst_tr xu (d,m,c,_) =
  let (r,c1) = rem_rsts c in
  let r = List.filter (fun x -> String.get x 1 = 'r') r in
  let (u,c2) = rem_unch c1 in
  let u = List.filter (fun x -> String.get x 1 = 'u') u in
  let nr = List.filter (fun c -> not (List.mem c u)) xu in
  (d,m,c2,List.sort_uniq compare (r@nr))
let add_rst_st xu (s,i,tl) = (s,i, List.map (add_rst_tr xu) tl)
let add_rst ((n,t,sl) as a) =
  let xu = get_unch a in
  (n,t,List.map (add_rst_st xu) sl)
...
let mk_or c1 c2 =
  let c = if c1=c2 then c1
          else if weaker c1 c2 then c1
          else if weaker c2 c1 then c2
          else Or(c1,c2) in
(*
  Printf.fprintf stderr "WEAKER: %a %a ==> %b,%b\n" pp_ctr c1 pp_ctr c2 (weaker c1 c2) (weaker c2 c1);
 *)
  c
